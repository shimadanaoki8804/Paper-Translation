        ...
        with st.form("question_form"):
            ...
            if st.form_submit_button("送信"):
                if user_input:  # user_inputが空でないかチェック
                    ...
                    #質問に関連したドキュメントを取得
                    search_retriever = Chroma.from_documents(documents, embedding=embeddings).as_retriever(
                                        search_kwargs={"k": 10}
                                        )
                    search_docs = search_retriever.get_relevant_documents(user_input)
                    reordered_docs = reordering.transform_documents(search_docs)

                    #レトリーバーの作成
                    retriever = Chroma.from_documents(reordered_docs, embeddings).as_retriever()
